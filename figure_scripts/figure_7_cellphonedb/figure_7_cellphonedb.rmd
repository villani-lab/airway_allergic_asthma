...
After aggregating the data, we wanted to see which interacting pairs were most specific to each group.  Below we show
the top 25 pairs for each disease group.  We also show this as a circos diagram (figure 7A).  This was done with
Javascript (D3) and the code for that can be found in the "figure 7a" folder of the repository.


```{r side_by_side_bar, fig.width = 10, fig.height = 10}
...
```{r th2_dotplot, fig.width = 8, fig.height = 9}
...
```{r th2_means, fig.width = 8, fig.height = 9}
...
```{r cellphonedb_basal, fig.width = 15, fig.height = 9}
...
```{r cellphonedb_goblet, fig.width = 15, fig.height = 9}
...
```{r goblet_means, fig.width = 15, fig.height = 9}
