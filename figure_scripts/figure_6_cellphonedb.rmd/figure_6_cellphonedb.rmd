...
With the outputs, we next wanted to aggregate the significant interactions and compare what we see with AA and AC.  One
drawback of cellphonedb is that it does not have great type-I error control.  Therefore, we set additional criteria when
determining what interactions to consider significant.  The criteria were as follows :
-imperical p-value <0.001
-receptor in > 10% of receptor cells
-receptor in > 20 cells total
-ligand in > 10% of ligand cells
-ligand in > 20 total cells

```{r aggregate_cellphedb, warning=FALSE, message=FALSE}
library(tidyverse)
library(glue)
library(magrittr)
...
```{r cellphonedb_heatmap, warning = FALSE, message = FALSE, fig.width = 8, fig.height = 7}
...
Next, we wanted to focus in on some interactions of biological interest.  We curated a list specific to Th2 interactions.
Given Th2 cells are predominantly found in the AA subjects, we displayed the significant ones for the AA only in a dot
plot.  A large point means it meets our statistical criteria listed at the top.  The rank is a value produced by
CellPhoneDB and represents how specific an interaction is to given clusters.  The more cluster pairs an interaction is
found to be significant, the lower the rank.  We took the -log10(rank) such that darker points represent interactions
that were found in less cluster pairs.


```{r th2_dotplot, fig.width = 8, fig.height = 7}
...
```{r th2_means, fig.width = 8, fig.height = 7}
...
Next, we wanted to look at some interactions between basal and myeloid cells.  Our prior analyses point at a strong role
for basal cells to be playing in the pathology of allergic asthma.  Here, we curated a list of interactions that we
think could be biologically important.

```{r cellphonedb_basal, fig.width = 15, fig.height = 7}
...
We were also interested in some of these interactions with the goblet cells.  Here, we show similar plots, between
goblet cells and myeloid cells.

```{r cellphonedb_goblet, fig.width = 15, fig.height = 7}
...
One important thing to consider when performing CellPhoneDB is that it dosen't look at interactions on a per-donor
basis.  Therefore, we wanted to look at the expression of the ligands/receptors of interest on a per-donor basis to
ensure signatures were not being driven by a single donor.  Below are plots that show the % of myeloid cells that
express the myeloid genes and % basal cells that express the basal genes. We can see that for the most part, results are
consistent across subject groups.


```{python cellphonedb_by_subj}
