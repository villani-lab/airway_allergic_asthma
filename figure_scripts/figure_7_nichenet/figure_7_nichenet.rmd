...
We can visualize the top AA ligand:downstream target with a heatmap.  The inside of the heatmap reflects the regulatory
potential as determined by Nichenet.  The outside reflects the p-values from our own DEG analysis to indicate which
clusters the given genes were significant in
...
```{r aa_myl_to_epi_circos, message=FALSE, warning=FALSE, fig.width = 8, fig.height = 8}
...
```{r ac epi_to_myl_circos, fig.width = 8, fig.height = 8}
circ_data <- setNames(reshape2::melt(as.matrix(merged_pot_mtx)), c('to', 'from', 'value'))
transparency = circ_data %>% mutate(weight =(value-min(value))/(max(value)-min(value))) %>% mutate(transparency = 1-weight) %>% .$transparency
